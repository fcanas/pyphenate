 ...
 # 
 #    Adapted by Fabian Canas, October 2009 from code by
 #    Ned Batchelder, July 2007.
 # 
 #    Ned Batchelder's Python code is in the public domain.
...
class Pyphenate(HTMLParser):
  ...
  def handle_data(self, data):
    try:
      top, self.contextStack = self.contextStack
      self.contextStack = (top, self.contextStack)
    except:
      self.stack.append(data)
      return
    if top == 'p':
      wordList = data.rsplit()
      nWordList = []
      data = ''
      for word in wordList:
        nWordList.append('&shy;'.join(self.hyphenator.hyphenate_word(word)))
      data = ' '.join(nWordList)
    self.stack.append(data)
